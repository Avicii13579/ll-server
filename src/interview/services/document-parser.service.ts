...
export class DocumentParserService {
  ...
  async parseDocumentFromUrl(url: string): Promise<string> {
    try {
      ...
      this.logger.error(`文档解析失败: ${errorMessage}`, errorStack);
      throw error;
    }
  }
  ...
  private async parsePdf(buffer: Buffer): Promise<string> {
    try {
      ...
      this.logger.log(
        `PDF 解析成功: 页数=${data.total}, 长度=${data.text.length}`,
      );
      ...
      this.logger.error(`PDF 解析失败: ${errorMessage}`);
      throw new BadRequestException(
        `PDF 文件解析失败: ${errorMessage}。请确保文件未加密且未损坏`,
      );
    }
  }
  ...
  private async parseDocx(buffer: Buffer): Promise<string> {
    try {
      ...
      // 其他错误记录详细日志并抛出通用错误提示
      this.logger.error(
        `DOCX 解析失败: ${err.message || '未知错误'}`,
        err.stack,
      );
      throw new BadRequestException(
        `DOCX 文件解析失败: ${err.message || '未知错误'}。请确保文件格式正确且未损坏`,
      );
    }
  }
